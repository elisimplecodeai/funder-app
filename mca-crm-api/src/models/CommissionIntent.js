...
const { INTENT_STATUS, PAYMENT_METHOD, COMMISSION_STATUS } = require('../utils/constants');

const CommissionIntentSchema = new mongoose.Schema({
    funding: {
...
// Helper function to calculate statistics for CommissionIntent documents
const calculateStatistics = async function(docs) {
    if (!docs || docs.length === 0) return;
    
    const Commission = mongoose.model('Commission');
    
    // Get all commission intent IDs
    const intentIds = docs.map(doc => doc._id);
    
    // Fetch all commissions for these plans in one query
    const commissions = await Commission.find({
        commission_intent: { $in: intentIds }
    }, 'commission_intent status amount');
    
    // Group commissions by intent ID and calculate statistics
    const statsByIntent = {};

    // Initialize stats for each intent to make sure we have stats for all intents, even if there are no commissions for that intent
    ...
    commissions.forEach(commission => {
        const intentId = commission.commission_intent.toString();
        
        const stats = statsByIntent[intentId];
        switch (commission.status) {
        case COMMISSION_STATUS.SUBMITTED:
            stats.submitted_count += 1;
            stats.submitted_amount += commission.amount || 0;
            break;
        case COMMISSION_STATUS.PROCESSING:
            stats.processing_count += 1;
            stats.processing_amount += commission.amount || 0;
            break;
        case COMMISSION_STATUS.SUCCEED:
            stats.succeed_count += 1;
            stats.succeed_amount += commission.amount || 0;
            break;
        case COMMISSION_STATUS.FAILED:
            stats.failed_count += 1;
            stats.failed_amount += commission.amount || 0;
            break;
        }
    });
...
CommissionIntentSchema.post('find', async function(docs) {
    if (this.getOptions()?.calculate) {
        await calculateStatistics(docs);
    }
});

CommissionIntentSchema.post('findOne', async function(doc) {
    if (doc && this.getOptions()?.calculate) {
        await calculateStatistics([doc]);
    }
});

const CommissionIntent = mongoose.model('Commission-Intent', CommissionIntentSchema);

module.exports = CommissionIntent;
