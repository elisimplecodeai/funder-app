...
import Step3UserRoles from './steps/Step3UserRoles';
import Step4CombinedSync from './steps/Step4CombinedSync';
...
interface ImportState {
  ...
  userRolesFetched: boolean;
  combinedSyncCompleted: boolean;
  onyxSettings: any;
  userRoles: any[];
}
...
const IMPORT_STEPS = [
  ...
  { id: 2, title: 'User Roles', component: Step3UserRoles },
  { id: 3, title: 'Sync Data', component: Step4CombinedSync },
...
export default function OnyxIQImportClient() {
  ...
  const canProceedToNextStep = () => {
    switch (importState.currentStep) {
      ...
      case 2: // User Roles step
        return importState.userRolesFetched;
      case 3: // Combined Sync step
        return importState.combinedSyncCompleted;
  ...
  const getStepRequirementMessage = () => {
    switch (importState.currentStep) {
      ...
      case 2:
        return 'Please fetch user roles before proceeding';
      case 3:
        return 'Please complete data sync before proceeding';
      case 4:
  ...
  return (
    <main className="flex flex-col min-h-screen w-full bg-custom-blue">
      ...
      {/* Sync Status */}
      {(importState.currentStep >= 3 && importState.currentStep <= 4) && (
        <div className="w-full bg-gray-50 border-t border-b">
          <div className="max-w-6xl mx-auto px-6 py-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-3 h-3 rounded-full ${importState.combinedSyncCompleted ? 'bg-green-500' : 'bg-yellow-500'}`} />
                <span className="text-sm font-medium text-gray-700">
                  Sync Status: {importState.combinedSyncCompleted ? 'Data sync completed' : 'Syncs in progress'}
                </span>
              </div>
