...
interface SyndicatorCreateStep3Props {
...
export default function SyndicatorCreateStep3({
  values,
  setFieldValue,
  onNext,
  onBack,
  loading
}: SyndicatorCreateStep3Props) {
  ...
  useEffect(() => {
    const fetchFunders = async () => {
      ...
      try {
        const response = await getFunderList();
        const uniqueFunders = response.filter((funder: Funder, index: number, self: Funder[]) => 
          index === self.findIndex((f: Funder) => f._id === funder._id)
        );
  ...
  const funderOptions = funders.map(funder => ({
    value: funder._id,
    label: funder.name
  }));

  const selectedFunders = values.funder_list?.map(funderId => 
    funderOptions.find(option => option.value === funderId)
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <Select
          ...
          onChange={(selected) => {
            const funderIds = selected ? selected.map(option => option.value) : [];
            setFieldValue('funder_list', funderIds);
          }}
          className="text-sm"
        ...
        <div className="text-xs text-gray-500 mt-1">
          Select one or more funders to associate with this syndicator
        </div>
      </div>
      ...
      <div className="flex justify-between pt-6 border-t border-gray-200">
        ...
        >
          {loading ? 'Creating...' : 'Create Syndicator'}
        </button>
      </div>
    </div>
  );
