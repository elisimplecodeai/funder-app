...
export function CreateModal({ isOpen, onClose, onSuccess, initialFundingId }: CreateModalProps) {
  ...
  // Set initial funding when initialFundingId is provided
  useEffect(() => {
    if (initialFundingId && formData.funding !== initialFundingId) {
      setFormData(prev => ({
        ...prev,
        funding: initialFundingId
      }));
    }
  }, [initialFundingId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only include expired_date if it has a value
      if (formData.expired_date) {
        cleanData.expired_date = formData.expired_date;
      }
