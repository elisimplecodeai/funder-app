...
const StatusSelectorWrapper = ({ item, onUpdate }: { item: ApplicationStipulation; onUpdate: (updatedItem: ApplicationStipulation) => void }) => {
    ...
    const [currentStatus, setCurrentStatus] = useState<ApplicationStipulationStatus>(item.status);

    const handleStatusUpdate = async (newStatus: ApplicationStipulationStatus) => {
        if (isLoading) return;
        setIsLoading(true);
        try {
    ...
    return (
        <StatusSelector<ApplicationStipulationStatus>
            value={currentStatus}
            ...
            confirmationStatuses={['WAIVED', 'VERIFIED']}
        />
    );
};
