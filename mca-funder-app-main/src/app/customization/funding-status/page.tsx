...
import { getFundingStatusListWithDefaultSort, updateFundingStatusOrder } from '@/lib/api/fundingStatuses';
import CreateFundingStatusForm from './_components/CreateFundingStatusForm';
import UpdateFundingStatusForm from './_components/UpdateFundingStatusForm';
import FundingStatusDetailModal from './_components/FundingStatusDetailModal';
import {
  PlusIcon,
  XMarkIcon,
...
interface SortableStatusItemProps {
  ...
  onView: (status: FundingStatus) => void;
}

function SortableStatusItem({ id, status, onView }: SortableStatusItemProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <div
      ...
      className={`flex items-center justify-between p-4 border border-theme-border rounded-lg shadow-theme-sm hover:shadow-theme-md transition-shadow cursor-pointer ${isDragging ? 'opacity-50' : ''
    ...
    >
      ...
      <div className="flex items-center space-x-2 text-xs">
        ...
        {status.funded && (
          <div className="px-2 py-1 rounded-full border inline-block" style={{ backgroundColor: 'rgba(59, 130, 246, 0.4)', color: '#2563eb', borderColor: 'rgba(59, 130, 246, 0.3)' }}>Funded</div>
        )}
        {status.performing && (
          <div className="px-2 py-1 rounded-full border inline-block" style={{ backgroundColor: 'rgba(34, 197, 94, 0.4)', color: '#059669', borderColor: 'rgba(34, 197, 94, 0.3)' }}>Performing</div>
        )}
        {status.warning && (
          <div className="px-2 py-1 rounded-full border inline-block" style={{ backgroundColor: 'rgba(245, 158, 11, 0.4)', color: '#d97706', borderColor: 'rgba(245, 158, 11, 0.3)' }}>Warning</div>
        )}
        {status.closed && (
        ...
        {status.defaulted && (
          <div className="px-2 py-1 rounded-full border inline-block" style={{ backgroundColor: 'rgba(239, 68, 68, 0.4)', color: '#dc2626', borderColor: 'rgba(239, 68, 68, 0.3)' }}>Defaulted</div>
        )}
        {status.system && (
...
export default function FundingStatusPage() {
  ...
  const [editingStatus, setEditingStatus] = useState<FundingStatus | null>(null);
  const [viewingStatus, setViewingStatus] = useState<FundingStatus | null>(null);
  const [hasUnsavedChanges, setHasUnsavedChanges] = useState(false);
  const [originalOrder, setOriginalOrder] = useState<FundingStatus[]>([]);
  ...
  const fetchStatuses = useCallback(async () => {
    ...
    try {
      ...
      const data = await getFundingStatusListWithDefaultSort(funder._id, includeInactive);
      const sortedData = data.sort((a: FundingStatus, b: FundingStatus) => a.idx - b.idx);
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      ...
      await updateFundingStatusOrder(orderData);
  ...
  const handleUpdateSuccess = (updatedStatus: FundingStatus) => {
  ...
  const handleView = (status: FundingStatus) => {
    setViewingStatus(status);
  };

  const handleEdit = (status: FundingStatus) => {
    setEditingStatus(status);
    setShowUpdateForm(true);
  };

  if (!accessToken) {
    return <div className="p-4 text-theme-foreground">Please log in to view funding statuses.</div>;
  }
    ...
    <FunderGuard>
      <div className='w-full border-theme-border rounded-xl p-8 bg-theme-accent'>
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl font-bold text-theme-primary">Funding Status</h1>
            {funder && (
              ...
              <label className="flex items-center space-x-3 cursor-pointer group">
                ...
                <div className="relative">
                  <input
                    ...
                    className="sr-only"
                  />
                  <div className={`block w-10 h-6 rounded-full transition ${includeInactive ? 'bg-theme-primary' : 'bg-theme-muted'
                    }`}>
                    <div className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition transform ${includeInactive ? 'translate-x-4' : 'translate-x-0'
                      }`}></div>
                  </div>
                </div>
              </label>
        ...
        {error && (
          <div className="mb-4 p-4 bg-error/10 border border-error/20 rounded-lg flex items-center justify-between">
            ...
            <button
              ...
              className="ml-4 text-error hover:text-error/80 transition"
            >
        ...
        <div className="space-y-3">
          ...
          {statuses.length === 0 && !loading && (
            <div className="text-center py-12">
              ...
              <h3 className="text-theme-foreground font-medium mb-2">No funding statuses found</h3>
              <p className="text-theme-muted mb-4">Get started by creating your first funding status.</p>
              <button
                onClick={() => setShowCreateForm(true)}
                className="px-4 py-2 rounded-md bg-theme-primary text-white hover:bg-theme-primary/80 transition"
              >
