...
interface FilterProps {
  ...
  flatColumns: ColumnConfig<Formula>[];
}
...
export default function Filter({ value, onChange, visibleColumns, flatColumns }: FilterProps) {
  ...
  return (
    <div className="absolute top-full left-0 mt-2 p-4 border rounded-md bg-white shadow-md space-y-4 z-10 text-sm">
      ...
      {/* Include Private */}
      <div className="flex items-center gap-2">
        <label htmlFor="includePrivate" className="min-w-[100px] text-gray-700">Show Private:</label>
        <input
          id="includePrivate"
          type="checkbox"
          checked={localFilter.include_private}
          onChange={(e) => setLocalFilter({ ...localFilter, include_private: e.target.checked })}
          className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
        />
      </div>
