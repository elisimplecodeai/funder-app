...
export default function Filter({
  ...
  flatColumns: ColumnConfig<Contact>[];
}) {
  ...
  const nonSortableKeys = ['address', 'online', 'inactive'];
    ...
    <div className="absolute top-full left-0 mt-2 p-4 border rounded-md bg-white shadow-md space-y-4 z-10 text-sm">
      ...
      <div className="pt-2 flex justify-end gap-2">
        <button
          ...
          onClick={() => setLocalFilter({ sortBy: '', sortOrder: 'asc', include_inactive: false, search: null, user: [] })}
        >
          Clear
        </button>
        <button
