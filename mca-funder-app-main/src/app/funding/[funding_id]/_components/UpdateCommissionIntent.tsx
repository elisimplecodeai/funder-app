...
import { updateCommissionIntent } from '@/lib/api/commissionIntents';
import type { UpdateCommissionIntent, CommissionIntent } from '@/types/commissionIntent';
...
interface UpdateCommissionIntentProps {
...
export default function UpdateCommissionIntent({ commissionIntent, onSuccess, onCancel }: UpdateCommissionIntentProps) {
  ...
  return (
    <Formik
      initialValues={{
        ...
        commission_date: format(new Date(commissionIntent.commission_date), 'yyyy-MM-dd'),
        amount: commissionIntent.amount,
        payment_method: commissionIntent.payment_method,
        ach_processor: commissionIntent.ach_processor || 'ACHWorks',
        status: commissionIntent.status,
        note: commissionIntent.note || '',
      }}
      validationSchema={validationSchema}
      onSubmit={async (values, { setSubmitting, setStatus }) => {
        try {
          const commissionIntentData: UpdateCommissionIntent = {
          ...
          await updateCommissionIntent(commissionIntent._id, commissionIntentData);
          onSuccess();
        } catch (err: any) {
          setStatus(err.message || 'Failed to update commission intent');
        } finally {
          setSubmitting(false);
        }
      }}
