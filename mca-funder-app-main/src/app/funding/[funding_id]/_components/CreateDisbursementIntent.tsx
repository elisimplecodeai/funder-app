...
import type { CreateDisbursementIntent } from '@/lib/api/disbursementIntents';
...
interface CreateDisbursementIntentProps {
  fundingId: string;
  onSuccess: () => void;
  onCancel: () => void;
}
...
export default function CreateDisbursementIntent({ fundingId, onSuccess, onCancel }: CreateDisbursementIntentProps) {
  ...
  return (
    <Formik
      ...
      onSubmit={async (values, { setSubmitting, setStatus }) => {
        try {
          const disbursementIntentData: CreateDisbursementIntent = {
            funding: fundingId,
            ...
            note: values.note,
            status: values.status as 'SCHEDULED' | 'SUBMITTED' | 'SUCCEED' | 'FAILED' | 'CANCELLED',
          };
          await createDisbursementIntent(disbursementIntentData);
          onSuccess();
        } catch (err: any) {
          setStatus(err.message || 'Failed to create disbursement intent');
        } finally {
          setSubmitting(false);
        }
      }}
