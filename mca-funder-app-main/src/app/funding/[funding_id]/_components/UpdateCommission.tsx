...
import { updateCommission } from '@/lib/api/commissions';
import type { UpdateCommissionParams, Commission } from '@/types/commission';
...
import { BankAccount, AccountType } from '@/types/bankAccount';
...
// Helper function to extract account ID from embedded object or string
const extractAccountId = (account: any): string => {
  if (!account) return '';
  if (typeof account === 'string') return account;
  if (account && typeof account === 'object' && account._id) return account._id;
  if (account && typeof account === 'object' && account.id) return account.id;
  return '';
};

// Helper function to convert account to BankAccount format
const convertToBankAccount = (account: any): BankAccount => ({
  _id: account._id,
  name: account.name,
...
export default function UpdateCommission({ commission, onSuccess, onCancel }: UpdateCommissionProps) {
  ...
  // Extract account IDs for initial values
  const initialFunderAccountId = extractAccountId(commission.funder_account);
  const initialISOAccountId = extractAccountId(commission.iso_account);

  console.log('initialFunderAccountId', initialFunderAccountId);
  console.log('initialISOAccountId', initialISOAccountId);
  console.log('funderAccounts', funderAccounts);
  console.log('isoAccounts', isoAccounts);

  return (
    <Formik
      enableReinitialize
      initialValues={{
        funder_account: initialFunderAccountId,
        iso_account: initialISOAccountId,
        amount: commission.amount,
        submitted_date: commission.submitted_date ? format(new Date(commission.submitted_date), 'yyyy-MM-dd') : format(new Date(), 'yyyy-MM-dd'),
        responsed_date: commission.responsed_date ? format(new Date(commission.responsed_date), 'yyyy-MM-dd') : '',
        payment_method: commission.payment_method,
        ach_processor: commission.ach_processor,
      ...
      onSubmit={async (values, { setSubmitting, setStatus }) => {
        try {
          // Find the full account objects
          const funderAccount = funderAccounts.find(account => account._id === values.funder_account);
          const isoAccount = isoAccounts.find(account => account._id === values.iso_account);

          if (!funderAccount || !isoAccount) {
            throw new Error('Selected accounts not found');
          }

          const commissionData: UpdateCommissionParams = {
            funder_account: convertToBankAccount(funderAccount),
            iso_account: convertToBankAccount(isoAccount),
          ...
          await updateCommission(
            commission.commission_intent._id,
            commission._id,
            commissionData
          );
          onSuccess();
        } catch (err: any) {
          setStatus(err.message || 'Failed to update commission');
        } finally {
          setSubmitting(false);
        }
      }}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 items-start">
                <div>
                  ...
                  <Select
                    options={funderAccounts.map(account => ({
                      value: account._id,
                      label: account.name
                    }))}
                ...
                <div>
                  ...
                  <Select
                    options={isoAccounts.map(account => ({
                      value: account._id,
                      label: account.name
                    }))}
